...
class SonicYangPathMixin:
    ...
    def xpath_to_configdb_path(self, xpath: str, configdb: dict = None) -> str:
        ...
        """
        tokens = self.__get_xpath_tokens(xpath)
        if len(tokens) == 0:
            return ""
    ...
    def __get_xpath_tokens(self, xpath: str) -> list[str]:
        """
        ...
        tokens = []
        while idx < len(xpath):
            end = self.__get_xpath_token_end(idx+1, xpath)
    ...
    def __get_xpath_token_end(self, start: int, xpath: str) -> int:
        idx = start
        while idx < len(xpath):
            if xpath[idx] == "/":
                break
            elif xpath[idx] == "[":
                idx = self.__get_xpath_predicate_end(idx, xpath)
            idx = idx+1

        return idx


    def __get_xpath_predicate_end(self, start: int, xpath: str) -> int:
        idx = start
        while idx < len(xpath):
            if xpath[idx] == "]":
                break
            elif xpath[idx] == "'" or xpath[idx] == '"':
                idx = self.__get_xpath_quote_str_end(xpath[idx], idx, xpath)
    ...
    def __get_xpath_quote_str_end(self, ch: str, start: int, xpath: str) -> int:
